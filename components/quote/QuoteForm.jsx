...
function QuoteForm() {
  ...
  useEffect(() => {
    ...
    //TimeFrame Data Fatchinch
    request(process.env.NEXT_PUBLIC_DATA_API + "/service-timeframes").then(
      (data) => {
        const options = data.data.map((item) => ({
          value: item.id,
          label: item.title,
        }));
        setServiceDateOption(options);
      }
    );

    // Services Data Fatchinch
    request(process.env.NEXT_PUBLIC_DATA_API + "/services").then((data) => {
      const options = data.data.map((item) => ({
        value: item.id,
        label: item.title,
      }));
  ...
  const quoteSubmit = async (dataForm) => {
    ...
    request(process.env.NEXT_PUBLIC_DATA_API + '/quotes', 'POST', newData)
      .then((res) => {
        if (res.success) {
          reset()
          ...
          reset({ services: [], serviceDate: [], phone: '' })
        } else {
          if (res.errors) {
      ...
      <button
        ...
        className={
          disabledBtn
            ...
            : " relative [&>svg]:opacity-0 submit_btn h-[40px] w-full max-w-[348px] bg-[#0071DC] text-base font-semibold text-white duration-300 hover:opacity-70 mx-auto justify-center flex items-center"
        }
      >
        <svg
          aria-hidden="true"
